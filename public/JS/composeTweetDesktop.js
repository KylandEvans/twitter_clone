const tweet = {
	...
	addListeners() {
		...
		const submitTweetBtns = document.querySelectorAll("[data-submit-tweet]");

		tweetBox.addEventListener("focusin", tweet.textboxFocused);
		...
		submitTweetBtns.forEach(btn => {
			btn.addEventListener("click", tweet.submitTweet);
		});
	},
	...
	// Copied from SO
	endOfContentEditable(contentEditableElement) {
		var range, selection;
	...
	tweetBoxChange() {
		...
		const charWarningText = document.querySelector("[data-character-warning-text]");
		const outerCircle = document.querySelector("[data-outer-circle]");
		const innerCircle = document.querySelector("[data-inner-circle]");
		...
		charcount = tweetBox.innerText.length * 1.285714;
		if (tweetBox.innerText.length < 260) {
			progressBar.style.background = `conic-gradient(var(--twitter-blue) ${charcount}deg, var(--faded-grey) 0deg)`;
			if (outerCircle.classList.contains("outer-circle-warning")) {
				outerCircle.classList.remove("outer-circle-warning");
				outerCircle.classList.add("outer-circle");
			}
			if (innerCircle.classList.contains("inner-circle-warning")) {
				innerCircle.classList.remove("inner-circle-warning");
				innerCircle.classList.add("inner-circle");
			}
			if (!charWarningText.classList.contains("d-none")) {
				charWarningText.classList.add("d-none");
			}
			if (charWarningText.classList.contains("danger-text")) {
				charWarningText.classList.remove("danger-text");
				charWarningText.classList.add("faded-grey-text");
			}
		}

		if (tweetBox.textContent.length >= 260 && tweetBox.textContent.length < 280) {
			progressBar.style.background = `conic-gradient(var(--warning-color) ${charcount}deg, var(--faded-grey) 0deg)`;
			if (outerCircle.classList.contains("outer-circle")) {
				outerCircle.classList.remove("outer-circle");
				outerCircle.classList.add("outer-circle-warning");
			}
			if (innerCircle.classList.contains("inner-circle")) {
				innerCircle.classList.remove("inner-circle");
				innerCircle.classList.add("inner-circle-warning");
			}

			if (charWarningText.classList.contains("d-none")) {
				charWarningText.classList.remove("d-none");
			}
			...
			charWarningText.textContent = maxCharCount - tweetBox.textContent.length;
		}

		if (tweetBox.textContent.length >= 280) {
			progressBar.style.background = `conic-gradient(var(--danger-color) ${charcount}deg, var(--faded-grey) 0deg)`;
			if (outerCircle.classList.contains("outer-circle")) {
				outerCircle.classList.remove("outer-circle");
				outerCircle.classList.add("outer-circle-warning");
			}
			...
			if (charWarningText.classList.contains("faded-grey-text")) {
				charWarningText.classList.remove("faded-grey-text");
				charWarningText.classList.add("danger-text");
			}
			let val = tweetBox.textContent.slice(0, 280);
			tweetBox.textContent = "";
			tweetBox.textContent = val;
			tweet.endOfContentEditable(tweetBox);
			charWarningText.textContent = maxCharCount - tweetBox.textContent.length;
			charWarningText.classList.remove("animate--font-expand");
			charWarningText.classList.add("animate--font-expand");
			setTimeout(() => {
				charWarningText.classList.remove("animate--font-expand");
			}, 250);
		}
	...
	selectEveryone() {
		...
		const imgPath =
			"M12 1.75C6.34 1.75 1.75 6.34 1.75 12S6.34 22.25 12 22.25 22.25 17.66 22.25 12 17.66 1.75 12 1.75zm-.25 10.48L10.5 17.5l-2-1.5v-3.5L7.5 9 5.03 7.59c1.42-2.24 3.89-3.75 6.72-3.84L11 6l-2 .5L8.5 9l5 1.5-1.75 1.73zM17 14v-3l-1.5-3 2.88-1.23c1.17 1.42 1.87 3.24 1.87 5.23 0 1.3-.3 2.52-.83 3.61L17 14z";
		if (replyFollowCheck[0].classList.contains("selected")) {
		...
		if (replyEveryoneCheck[0].classList.contains("d-none")) {
	...
	selectFollow() {
		...
		if (replyFollowCheck[0].classList.contains("d-none")) {
	...
	selectMention() {
		...
		if (replyMentioncheck[0].classList.contains("d-none")) {
	...
	textboxFocused() {
		const hiddenItems = document.querySelectorAll("[data-default-hidden]");
	...
	submitTweet() {
		...
		const tweetTextInput = document.querySelector("[data-tweet-text]");
		const tweetBox = document.querySelector("[data-tweetBox]");
		...
		tweetTextInput.value = tweetBox.textContent;
		form.submit();
	},
};
